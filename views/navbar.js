  ...
  connectedCallback() {
    ...
    const getLinkClass = (pageName) => {
      ...
      if (activePage === pageName) {
        return baseClass + "text-ochre bg-ochre-light/30 md:bg-transparent md:border-b-2 md:border-ochre md:py-1";
      }
      return baseClass + "text-earth-600 hover:text-ochre hover:bg-alpine-muted md:hover:bg-transparent";
    };

    this.innerHTML = `
      <nav class="fixed top-0 w-full z-50 bg-alpine-bg/95 backdrop-blur-md shadow-sm border-b border-earth-100/30">
        <div class="flex justify-between items-center w-full px-4 md:px-8 max-w-screen-2xl mx-auto h-20 relative z-50">
          
          <!-- BRAND -->
          <a href="index.html" class="flex items-center gap-1 text-xl md:text-2xl font-black text-earth-900 font-headline tracking-tight whitespace-nowrap">
          ...
          <button id="mobile-menu-btn" class="md:hidden flex flex-col justify-center items-center w-10 h-10 space-y-1.5 focus:outline-none rounded-lg bg-alpine-muted p-2">
            <span class="w-6 h-0.5 bg-earth-900 rounded transition-all duration-300 origin-center" id="line1"></span>
            <span class="w-6 h-0.5 bg-earth-900 rounded transition-all duration-300" id="line2"></span>
            <span class="w-6 h-0.5 bg-earth-900 rounded transition-all duration-300 origin-center" id="line3"></span>
          </button>
        </div>

        <!-- MOBILE MENU OVERLAY -->
        <div id="mobile-menu" class="absolute top-20 left-0 w-full bg-alpine-bg z-40 hidden flex-col px-6 py-6 shadow-xl border-b border-earth-100/30">
          <div class="flex flex-col space-y-2 items-start w-full">
            <a class="${getLinkClass('home')}" href="index.html">Explore Trails</a>
            <a class="${getLinkClass('howitworks')}" href="howitworks.html">How It Works</a>
            <a class="${getLinkClass('aboutus')}" href="aboutus.html">About Us</a>
            
            <div class="w-full pt-6 mt-2 border-t border-earth-100/30">
